      ...
      Get["/response/conference"] = x => {
        ...
        resp.AddConference("demo", new Dictionary < string, string > () {
          ...
          }, // Used to play a sound when a member enters the conference
          {
            "record",
            "true"
          }, // Option to record the call
          {
            "action",
            "http://dotnettest.apphb.com/response/conf_action"
          }, // URL to which the API can send back parameters
          {
            "method",
            "GET"
          }, // method to invoke the action Url
          {
            "callbackUrl",
            "http://dotnettest.apphb.com/response/conf_callback"
