    ...
    public Program() {
      Get["/call_whisper"] = x => {
        ...
        Plivo.XML.Dial dial = new Plivo.XML.Dial(new Dictionary < string, string > () {
          {
            "confirmSound",
            "http://dotnettest.apphb.com/confirm_sound"
          }, // A remote URL fetched with POST HTTP request which must return an 
          // XML response with Play, Wait and/or Speak elements only.
          {
            "confirmKey",
            "5"
          } // The digit to be pressed by the called party to accept the call.
        });
